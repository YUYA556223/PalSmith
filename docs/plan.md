...
### F8. アンインストール安全網(2026-07-17追加)

- **背景**: 定義が消えたアイテム/設置物はセーブ内で不正化し、ワールドが激重/ロード不能になる。
  PalSchema 0.6.0×現行ビルドはinvalid item cleanupが無効(シグネチャ欠落)のため、根本対策が上流待ち
- **PalSmith側の計画**:
  1. ランタイムが各パックのコンテンツID一覧をディスクに記録(スナップショット)
  2. パックが消えていたら、記録から**墓標スタブ**(名前"Legacy item"・レシピなしの最小行)を自動生成して
     セーブをロード可能に保つ
  3. `purge <packId>` 的な管理コマンドでワールドから該当コンテンツを一括除去 → その後スタブも削除
- 実装はランタイムv0.2以降。現状はドキュメントの「安全なアンインストール手順」で運用回避
...
| # | 内容 | 検証すること | 状態 |
|---|---|---|---|
| PoC-A | PalSchemaのみで新アイテム+レシピ+PNGアイコン+ツリー解放([poc/PoC-A-newcontent](../deprecated/poc/PoC-A-newcontent)) | データ層をPalSchemaに全委譲できるか / `$resource`が専用鯖+クライアント構成で動くか / `DT_TechnologyRecipeUnlock`のアイテム用列名 | ✅ **クライアント検証成功(2026-07-16)** C1/C2実証。列名`UnlockItemRecipes`確定。残: マルチ構成+セーブ健全性 |
| **V1** | `buildings`+既存BP流用で新規配置物をJSONのみ追加([poc/V1-placeable](../deprecated/poc/V1-placeable)) | 「配置できるアイテム」がUE5なしで成立するか | ✅ **成功(2026-07-16)** Bメニュー出現・設置・機能OK。C3実証 |
| **V2** | アイテム使用イベントのフック特定([poc/V2-itemuse-hook](../deprecated/poc/V2-itemuse-hook)) | `onUse` Behaviorの生死。使用時に関数がフックできStaticItemIdが取れるか | ✅ **成功(2026-07-16)** `UseItemToCharacter_ServerInternal`のparam1からID直読み。残: 専用鯖での発火確認 |
| V3 | 配置物イベント(onPlace/onInteract)のフック特定([poc/V3-place-interact-hooks](../deprecated/poc/V3-place-interact-hooks)) | 建物系Behaviorの成立性 | ✅ **成功(2026-07-16)** 3/3発火。BuildObjectId直読み。**C4完全成立** |
| V4 | 最小WBPをcook→pakロード→Luaから開閉+クリック往復([poc/V4-ui](../deprecated/poc/V4-ui)) | UIフレームワーク(④)の土台 | プレイブック+F11プローブ配置済。UE5セットアップ(Part A/B)待ち |
| PoC-F | チャットトランスポート `smith://` の往復+表示抑制([poc/PoC-F-chatprotocol](../deprecated/poc/PoC-F-chatprotocol)) | `server_action`/ハンドシェイク(F5)の通信路 | スクリプト作成済・実機未検証 |
| **V5** | ProceduralMeshでキー入力から立方体をワールド生成([poc/V5-runtime-mesh](../deprecated/poc/V5-runtime-mesh)) | ランタイム3Dオブジェクト(⑦)の成立性: AddComponentByClass→CreateMeshSectionのLua呼び出し | ✅ **成功(2026-07-16)** 立方体出現。注意: 空`{}`Transform=スケール0の罠 |
