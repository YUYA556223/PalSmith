...
| 新規3Dモデルのランタイム読み込み | ❌ 存在しない | 原理的には可能だがエンジンABI依存で非現実的(§2「やらないこと」) |
...
- **新規3Dメッシュのランタイム読み込み**: glTFRuntime相当の再実装がUE4SS C++ Modで必要になり、
  エンジン内部ABI依存でゲームアップデート毎に壊れる。パルはSkeletalMesh+AnimBPで特に厳しい。
  → ランタイムではなく**ツールチェーンで解く**: PMKテンプレプロジェクト+ヘッドレスcook
  (`RunUAT BuildCookRun`)を自動化するCLI(C#、ModManagerの延長)で
  「FBX+manifest→コマンド一発→ `paks/` に.pak生成」。これは後続フェーズ。
...
| # | 内容 | 検証すること | 状態 |
|---|---|---|---|
| PoC-A | PalSchemaのみで新アイテム+レシピ+PNGアイコン+ツリー解放([poc/PoC-A-newcontent](../poc/PoC-A-newcontent)) | データ層をPalSchemaに全委譲できるか / `$resource`が専用鯖+クライアント構成で動くか / `DT_TechnologyRecipeUnlock`のアイテム用列名 | 雛形作成済・実機未検証 |
| **V1** | `buildings`+既存BP流用で新規配置物をJSONのみ追加([poc/V1-placeable](../poc/V1-placeable)) | 「配置できるアイテム」がUE5なしで成立するか | 雛形作成済 |
| **V2** | アイテム使用イベントのフック特定([poc/V2-itemuse-hook](../poc/V2-itemuse-hook)) | `onUse` Behaviorの生死。使用時に関数がフックできStaticItemIdが取れるか | 調査スクリプト作成済 |
