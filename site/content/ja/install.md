...
**ワールドにそのパックのコンテンツが残っている間は、パックを外さないでください。**
定義が消えたアイテム・設置物はセーブ内で*不正アイテム*になり、ワールドが
激重になったりロード不能になったりします(実体験済み)。

安全な手順:

1. そのパックを使った全ワールドで: 設置物を破壊し、アイテムを全インベントリ/
   チェストから破棄 → セーブして退出
2. その後で `PalSchema/mods/` からパックフォルダを削除

すでにこの状態に陥った場合は、**パックを再インストール**すればワールドは
再びロードできます。その後、正しい順序で片付けてください。
